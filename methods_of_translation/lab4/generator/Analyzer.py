...
class Analyzer:
    ...
    def __init__(self, file: str, name: str, path="", generate_code=True):
        ...
        self._tokens_union = dict()  # FIRST = { nonterm: tokens1 + tokens2 + ... -> set() }
    ...
    def _dfs(self, non_term: NonTerm):
        if non_term.name in self._tokens_union.keys():
            return self._tokens_union[non_term.name]
        ...
        self._tokens_union[non_term.name] = set()
        ...
        for rule in non_term.rules:
            ...
            assert _tokens & self._tokens_union[non_term.name] == set(), "Нарушение правила LL(1)"
            self._tokens_union[non_term.name].update(_tokens)
